...
require_once 'sellerpanel/includes/db.php';
...
<nav class="bg-primary text-white px-4 py-3 flex justify-between items-center sticky top-0 z-50 shadow-lg">
  ...
  <script>
    ...
    // Filter products based on selections (UPDATED)
    function filterProducts() {
      const maxPrice = parseFloat(document.getElementById('priceRange').value);
      const selectedCategories = Array.from(document.querySelectorAll('.category-filter:checked')).map(el => el.value.toLowerCase());
      const selectedBrands = Array.from(document.querySelectorAll('.brand-filter:checked')).map(el => el.value.toLowerCase());
      ...
      container.innerHTML = ''; // clear previous children

      if (filteredProducts.length > 0) {
        filteredProducts.forEach(product => {
          product.style.display = 'flex';
          container.appendChild(product); // Repositions the element
        });
      } else {
        container.innerHTML = `
          <div class="col-span-full text-center text-gray-500 py-12">
